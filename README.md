...
Обычные DNS-ответы сохраняют TTL источника. Если имя существует, но записи
нужного типа нет, клиент получает `NOERROR/NODATA`; отсутствующее имя даёт
`NXDOMAIN`. Оба ответа содержат `SOA` для отрицательного кеша. Основной
отрицательный ответ один раз перепроверяется через резервные DNS одинаково для
`A` и `AAAA`.

Домены из deny-списков получают `NXDOMAIN`. Клиент с поддержкой EDNS также
видит EDE `Blocked`; старые клиенты просто получают обычный `NXDOMAIN`.
...
Для подменных адресов IPv6 установщик по умолчанию включает альтернативную сеть
`2001:2::/48`, аналогично `198.18.0.0/15` для IPv4. Настройка сохраняется как
`ALTERNATIVE_FAKE_IPV6=y`. При выборе `n` используется ULA `/96`, производная
от `VPN_IPV6_PREFIX`.
