...
- При `DISABLE_IPV6=n` OpenVPN, WireGuard, AmneziaWG и внутренний DNS работают
  в dual-stack. Отдельного переключателя для этого нет.
- AAAA для доменов AntiZapret уходит через туннель.
- Full VPN получает IPv6 default route.
- Внутренний DNS доступен по IPv4 и ULA IPv6. Перехват TCP/UDP 53 одинаковый
  для обоих стеков.
...
IPv6-адрес DNS сервер выдаёт вместе с IPv4. На старых сборках RouterOS это надо
проверять на конкретном клиенте: урезать DNS push ради одного клиента сервер не
будет.
