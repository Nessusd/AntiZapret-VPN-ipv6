...
Принудительная блокировка IPv6 отключена. Полноценная маршрутизация IPv6 через VPN пока не реализована — IPv6-трафик может идти напрямую через провайдера, поэтому перед использованием проверьте отсутствие утечек IPv6\
...
2. Для установки или обновления в терминале под root выполнить
```
bash <(wget -qO- --no-hsts --inet4-only https://raw.githubusercontent.com/Nessusd/AntiZapret-VPN-ipv6/main/proxy.sh)
```
